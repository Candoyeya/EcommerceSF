        ...
        Try
            ...
            If Session("usdrate") > 0 Then
                ...
                Else
                    PaginationDiv.InnerHtml = ""
                End If
                'sqldato = "select  CONVERT(int,ROW_NUMBER() OVER ( ORDER BY T0.ItemCode asc)) AS 'RowNum' 			,T0.ItemCode 	 			,SUM(T1.OnHand - T1.IsCommited - T1.OnOrder) as 'disp'  			,T0.ItemName  			,ISNULL(T0.U_IL_iva, 'null') as 'iva' 			,ISNULL(CONVERT(VARCHAR(MAX), T0.U_IL_imagen),'') as 'imag' 	 , t0.InvntryUom		from OITM T0 			inner join OITW T1 on T0.ItemCode = T1.ItemCode  			inner join OWHS t3 on T1.WhsCode = T3.WhsCode 		where T3.WhsName = '" & wharehose & "' and(t1.ItemCode like  '%119182dusdui2%'  "
                sqldato = "select  CONVERT(int,ROW_NUMBER() OVER ( ORDER BY T0.ItemCode asc)) AS 'RowNum' 			,T0.ItemCode 	 			,SUM(T1.OnHand - T1.IsCommited - T1.OnOrder) as 'disp'  			,T0.ItemName  			,ISNULL(T0.U_IL_iva, 'null') as 'iva' 			,ISNULL(CONVERT(VARCHAR(MAX), T0.U_IL_imagen),'') as 'imag' 	 , t0.InvntryUom		from OITM T0 			inner join OITW T1 on T0.ItemCode = T1.ItemCode  			inner join OWHS t3 on T1.WhsCode = T3.WhsCode   	left join EcommerceSF.dbo.TAAE T4 On T4.ItemCode=T0.ItemCode	  where T3.WhsName = '" & wharehose & "' and(t1.ItemCode like  '%119182dusdui2%'  "
                pattern = " "
                elements = Regex.Split(Session("busqueda"), pattern)
                For Each element In elements
                    If element <> "" Then
                ...
                sqldato = sqldato & ") and t0.validFor='Y' and T0.U_IL_iva is not null group by  			T0.ItemCode 	 , t0.InvntryUom		   ,T0.ItemName  		   ,ISNULL(T0.U_IL_iva, 'null') 		   ,CONVERT(VARCHAR(MAX), T0.U_IL_imagen) 	 "
                ...
                '----Seccion carga de matriz articulos
                Respuesta = ws.ExecuteSQL(Session("Token"), sqldato)
                'XmlDoc.LoadXml(ws.ExecuteSQL(Session("Token"), sqldato).InnerXml)
                'Node = XmlDoc.FirstChild.LastChild.Clone.ChildNodes
                '--------->Desplegar nodos de consulta sql en xml
                            ...
                            For x = 0 To rootin.ChildNodes.Count - 1
                                Select Case rootin.ChildNodes(x).Name
                                    ...
                                    Case "imag"
                                        ...
                                        Else
                                            'twoDarray(s, x + 2) = Session("ftp") + "Imagenes/" + rootin.ChildNodes(x).InnerText + ".JPG"
                                            'twoDarray(s, x + 2) = "../../" + rootin.ChildNodes(x).InnerText

                                            XmlDoc.LoadXml(ws.ExecuteSQL(Session("Token"), "Select T0.ImagenBase From EcommerceSF.dbo.TAAE T0 WHERE T0.ItemCode='" & twoDarray(s, 1) & "'").InnerXml)
                                            Node = XmlDoc.FirstChild.LastChild.Clone.ChildNodes
                                            '--------->Desplegar nodos de consulta sql en xml
                                            For Each Nodo As XmlNode In Node
                                                twoDarray(s, x + 2) = Nodo("ImagenBase").InnerText
                                            Next
                                        End If
                                    Case "InvntryUom"
                                        twoDarray(s, x + 2) = rootin.ChildNodes(x).InnerText
    ...
    Public Sub Catalogo()
        ...
        Try
            ...
            Dim sqldato As String = "select T0.ItemCode,T0.ItemName,ISNULL(CONVERT(VARCHAR(MAX), T4.ImagenBase ),'') as 'imag' " &
                                    ...
                                    "where T3.WhsName = 'Ferreteria' and t0.validFor='Y' and T0.U_IL_iva is not null " &
                                    "group by T0.ItemCode,T0.ItemName,CONVERT(VARCHAR(MAX), T4.ImagenBase)"
